...
public class GetJsonChiTietXe extends AsyncTask<Void, Void, Void> {
    Activity activity;
    String url;
    ArrayList<Xe> list = new ArrayList<>();


    public GetJsonChiTietXe(Activity activity, String url, ArrayList<Xe> list) {
        list.clear();
    ...
    protected Void doInBackground(Void... voids) {
        ...
        if (json != null) {
            try {
                ...
                JSONArray array = object.getJSONArray("tbl_chitietxe");
                for (int i = 0; i < array.length(); i++) {
                    Xe xe = new Xe();
                    JSONObject jsonObject = array.getJSONObject(i);
                    String idxe = jsonObject.getString("idxe");
